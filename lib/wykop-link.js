...
module.exports = class Link extends API {
    ...
    upvote = async function({ id = this.id } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
        return this.wrapContent('none', this.#instance.post('/links/' + id + '/votes/up')).then(_ => this);
    }
    ...
    downvote = async function({ id = this.id, reason = null } = {}) {
        ...
        return this.wrapContent('none', this.#instance.post('/links/' + id + '/votes/down')).then(_ => this)
    }

    // Remove upvote or downvote from link
    unvote = async function({ id = this.id } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
        return this.wrapContent('none', this.#instance.delete('/links/' + id + '/votes')).then(_ => this)
    }

    favorite = async function(id = this.id) {
        assert(id, this.#errors.assert.notSpecified('id'));
        return this._favorite({ linkId: id }).then(_ => this);
    }

    unfavorite = async function(id = this.id) {
        assert(id, this.#errors.assert.notSpecified('id'));
        return this._unfavorite({ linkId: id }).then(_ => this);
    }

    toggleMuteAMA = async function({ id = this.id } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
        return this.wrapContent('none', this.#instance.get('/links/' + id + '/ama/start')).then(_ => this);
    }
