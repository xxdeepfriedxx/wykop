...
module.exports = class Link extends API {
    ...
    upvote = async function({ id = this.id } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
        return this.wrapContent('none', this.#instance.post('/links/' + id + '/votes/up')).then(_ => { return this });
    }
    ...
    downvote = async function({ id = this.id, reason = null } = {}) {
        ...
        return this.wrapContent('none', this.#instance.post('/links/' + id + '/votes/down')).then(_ => { return this })
    }

    // Remove upvote or downvote from link
    unvote = async function({ id = this.id } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
        return this.wrapContent('none', this.#instance.delete('/links/' + id + '/votes')).then(_ => { return this })
    }
    ...
    toggleMuteAMA = async function({ id = this.id } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
        return this.wrapContent('none', this.#instance.get('/links/' + id + '/ama/start')).then(_ => { return this });
    }
