...
module.exports = class Link extends API {
    ...
    get = async function({ id = this.id } = {}) {
        ...
        return this.wrapContent('link', await this.#instance.get('/links/' + id));
    }

    getComment = async function(commentId, { id = this.id } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
        assert(commentId, this.#errors.assert.notSpecified('commentId'));
        return this.wrapContent('link_comment', await this.#instance.get('/links/' + id + '/comments/' + commentId));
    }

    submitComment = async function({ id = this.id, commentId = null, content = null, photo = null, embed = null, adult = false } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
        return this.wrapContent('link_comment', await this.#instance.post('/links/' + id + '/comments' + (commentId ? '/' + commentId : ''), {
            data: {
                content: content,
                photo: photo,
                embed: embed,
    ...
    getComments = async function({ id = this.id, commentId = null, page = null, sort = null, ama = null } = {}) {
        ...
        if (commentId) {
            return this.wrapListing('link_comment', await this.#instance.get('/links/' + id + '/comments/' + commentId + '/comments', {
                params: {
                    page: page
                }
            }));
        }
        return this.wrapListing('link_comment', await this.#instance.get('/links/' + id + '/comments', {
            params: {
                page: page,
                sort: sort,
                ama: ama
    ...
    getRelatedLinks = async function({ id = this.id } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
        return this.wrapListing('related', await this.#instance.get('/links/' + id + '/related'));
    }
    ...
    submitRelatedLink = async function({ id = this.id, title = null, url = null, adult = false } = {}) {
        ...
        return this.wrapContent('related', await this.#instance.post('/links/' + id + '/related', {
            data: {
                title: title,
                url: url,
                adult: adult
    ...
    edit = async function({ id = this.id, title = this.title, description = this.description, tags = this.tags, photo = this.media?.photo?.key, adult = this.adult } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
        return this.wrapContent('none', await this.#instance.put('/links/' + id, {
            data: {
                title: title,
    ...
    remove = async function({ id = this.id } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
        return await this.#instance.delete('/links/' + id);
    }

    // Upvote a link
    upvote = async function({ id = this.id } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
        return await this.#instance.post('/links/' + id + '/votes/up').then(_ => { return this });
    }
    ...
    downvote = async function({ id = this.id, reason = null } = {}) {
        ...
        return await this.#instance.post('/links/' + id + '/votes/down').then(_ => { return this })
    }

    // Remove upvote or downvote from link
    unvote = async function({ id = this.id } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
        return await this.#instance.delete('/links/' + id + '/votes').then(_ => { return this })
    }
    ...
    getUpvotes = async function({ id = this.id } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
        return this.wrapListing('none', await this.#instance.get('/links/' + id + '/upvotes/up')).then(res => {
    ...
    getDownvotes = async function({ id = this.id } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
        return this.wrapListing('none', await this.#instance.get('/links/' + id + '/upvotes/down')).then(res => {
            return res.items.map(res => {
                res.user = new Profile(this.#core, res.user)
                return res
            });
    ...
    getCounters = async function({ id = this.id } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
        return this.wrapContent('none', await this.#instance.get('/links/' + id + '/counters'));
    }

    // Get the redirect URL for a link
    getRedirectURL = async function({ id = this.id } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
        return this.wrapContent('none', await this.#instance.get('/links/' + id + '/redirect')).then(res => res.url);
    }
}