...
export default class Link extends API {
    ...
    constructor(core, data) {
        ...
        if (this.comments?.items) { this.comments.items = this.comments.items.map(x => { return new LinkComment(this.#core, x) }) }
    ...
    submitComment = function({ id = this.id, commentId = null, content = null, photo = null, embed = null, adult = false } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
    ...
    getComments = function({ id = this.id, commentId = null, page = null, sort = null, ama = null } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
    ...
    getRelatedLinks = function({ id = this.id } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
        return this.wrapListing('related', this.#instance.get('/links/' + id + '/related'));
    }

    submitRelatedLink = function({ id = this.id, title = null, url = null, adult = false } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
    ...
    edit = function({ id = this.id, title = this.title, description = this.description, tags = this.tags, photo = this.media?.photo?.key, adult = this.adult } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
    ...
    // Downvote a link
    downvote = function({ id = this.id, reason = null } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
    ...
    getUpvotes = function({ id = this.id } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
    ...
    getDownvotes = function({ id = this.id } = {}) {
        assert(id, this.#errors.assert.notSpecified('id'));
