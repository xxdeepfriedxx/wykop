...
module.exports = class Link extends API {
	...
	constructor(core, data, overrideProxy = false) {
		...
		if (this.source?.type === 'article') { this.article = this.wrapContent('article', { id: this.source.type_id }, null, true); }
	...
	observe = async function({ id = this.id } = {}) {
		assert(id, this.#errors.assert.notSpecified('id'));
		return this.wrapContent('none', this.#instance.post('/links/' + id + '/observed-discussions')).then(() => this);
	};

	unobserve = async function({ id = this.id } = {}) {
		assert(id, this.#errors.assert.notSpecified('id'));
		return this.wrapContent('none', this.#instance.delete('/links/' + id + '/observed-discussions')).then(() => this);
	};
	...
	startAMA = async function({ id = this.id } = {}) {
		assert(id, this.#errors.assert.notSpecified('id'));
	...
	muteAMA = async function({ id = this.id } = {}) {
		assert(id, this.#errors.assert.notSpecified('id'));
		return this.wrapContent('none', this.#instance.get('/links/' + id + '/ama/mute')).then(() => this);
	};

	unmuteAMA = async function({ id = this.id } = {}) {
		assert(id, this.#errors.assert.notSpecified('id'));
		return this.wrapContent('none', this.#instance.get('/links/' + id + '/ama/unmute')).then(() => this);
	};
