...
module.exports = class Pagination extends API {
	...
	#setupPages = function() {
		if (this.#config && this.#config.params && this.#config.params.page) {
			this.currentPage = this.#config.params.page;
		}

		this.get = this.#requestPageFunction(this.currentPage);
	...
	get #prevPageValue() {
		...
		if (this.#pageType === 'numerical') { 
			return (this.currentPage - 1) < 1 ? null : this.currentPage - 1;
		}
		return null;
	}
};