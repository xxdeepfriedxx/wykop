...
module.exports = class Profile extends API {
	...
	getLinksRelated = async function({ username = this.username, page = null } = {}) {
		assert(username, this.#errors.assert.notSpecified('username'));
		return this.wrapListing('link_related', this.#instance.get('/profile/users/' + username + '/links/related', { params: { page: page }}));
	};
