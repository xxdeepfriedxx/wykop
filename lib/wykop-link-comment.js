...
export default class LinkComment extends API {
	...
	addComment = function({ id = this.id, linkId = this.parent?.id, content = this.content, photo = this.media?.photo?.key, embed = this.media?.embed?.key, adult = this.adult ?? false } = {}) {
		assert(id, this.#errors.assert.notSpecified('id'));
		assert(linkId, this.#errors.assert.notSpecified('linkId'));
