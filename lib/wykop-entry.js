...
module.exports = class Entry extends API {
	...
	upvote = async function({ id = this.id } = {}) {
		assert(id, this.#errors.assert.notSpecified('id'));
		return this.wrapContent('none', this.#instance.post('/entries/' + id + '/votes')).then(_ => { return this })
	}

	unvote = async function({ id = this.id } = {}) {
		assert(id, this.#errors.assert.notSpecified('id'));
		return this.wrapContent('none', this.#instance.delete('/entries/' + id + '/votes')).then(_ => { return this })
	}
    ...
    surveyVote = async function(id = null) {
		assert(id, this.#errors.assert.notSpecified('id'));
        return this.wrapContent('none', this.#instance.post('/entries/' + id + '/survey/votes')).then(_ => { return this })
    }
}