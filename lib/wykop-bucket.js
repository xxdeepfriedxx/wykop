...
module.exports = class Bucket extends API {
    ...
    getContent = async function({ hash = this.hash, page = null} = {}) {
        return this.wrapListingMixed(await this.#instance.get('/buckets/stream/' + hash, {
            params: {
                page: page
            }
        }));
    ...
    edit = async function({ hash = this.hash, title = this.title, query = this.query } = {}) {
        return this.wrapContent('bucket', await this.#instance.put('/buckets/' + hash, {
            data: {
                title: title,
                query: query
            }
    ...
    remove = async function({ hash = this.hash } = {}) {
        return await this.#instance.delete('/buckets/' + hash)
    }
}