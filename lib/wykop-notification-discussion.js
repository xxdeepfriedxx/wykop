...
module.exports = class DiscussionNotification extends API {
	...
	get = async function({ id = this.id } = {}) {
		...
		return this.wrapContent('notification_discussion', this.#instance.get('/notifications/observed-discussions/' + id));
	};

	markAsRead = async function({ id = this.id } = {}) {
		assert(id, this.#errors.assert.notSpecified('id'));
		return this.wrapContent('none', this.#instance.put('/notifications/observed-discussions/' + id));
	};

	remove = async function({ id = this.id } = {}) {
		assert(id, this.#errors.assert.notSpecified('id'));
		return this.wrapContent('none', this.#instance.delete('/notifications/observed-discussions/' + id));
	};
};