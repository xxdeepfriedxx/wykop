...
module.exports = class Tag extends API {
	...
	get = async function({ tag = this.name } = {}) {
		...
		return this.wrapContent('tag', await this.#instance.get('/tags/' + tag));
	}
	...
	getContent = async function({ tag = this.name, page = null, sort = null, type = null, year = null, month = null } = {}) {
		...
		assert(tag, this.#errors.assert.notSpecified('tag'));
		return this.wrapListingMixed(await this.#instance.get('/tags/' + tag + '/stream', {
			params: {
				page: page,
				sort: sort,
				type: type,
	...
	getNewerContent = async function({ tag = this.name, sort = null, type = null, date = null, lastId = null } = {}) {
		...
		return this.wrapListingMixed(await this.#instance.get('/tags/' + tag + '/newer', {
			params: {
				sort: sort,
				type: type,
				date: date,
	...
	getRelatedTags = async function({ tag = this.name } = {}) {
		assert(tag, this.#errors.assert.notSpecified('tag'));
		return this.wrapListing('tag', await this.#instance.get('/tags/' + tag + '/related'));
	}

	getAuthors = async function({ tag = this.name } = {}) {
		assert(tag, this.#errors.assert.notSpecified('tag'));
		return this.wrapListing('profile', await this.#instance.get('/tags/' + tag + '/users'));
	}

	addAuthor = async function({ tag = this.name, username = null } = {}) {
		assert(tag, this.#errors.assert.notSpecified('tag'));
		assert(username, this.#errors.assert.notSpecified('username'));
		return await this.#instance.post('/tags/' + tag + '/users/' + username).then(_ => { return this });
	}

	removeAuthor = async function({ tag = this.name, username = null } = {}) {
		assert(tag, this.#errors.assert.notSpecified('tag'));
		assert(username, this.#errors.assert.notSpecified('username'));
		return await this.#instance.delete('/tags/' + tag + '/users/' + username).then(_ => { return this });
	}

	edit = async function({ tag = this.name, photo = null, description = null } = {}) {
		assert(tag, this.#errors.assert.notSpecified('tag'));
		return await this.#instance.get('/tags/' + tag, {
			params: {
				photo: photo,
	...
	observe = async function({ tag = this.name } = {}) {
		assert(tag, this.#errors.assert.notSpecified('tag'));
		return await this.#instance.post('/observed/tags/' + tag).then(_ => { return this });
	}

	unobserve = async function({ tag = this.name } = {}) {
		assert(tag, this.#errors.assert.notSpecified('tag'));
		return await this.#instance.delete('/observed/tags/' + tag).then(_ => { return this });
	}

	notify = async function({ tag = this.name } = {}) {
		assert(tag, this.#errors.assert.notSpecified('tag'));
		return await this.#instance.put('/observed/tags/' + tag + '/notifications').then(_ => { return this });
	}

	mute = async function({ tag = this.name } = {}) {
		assert(tag, this.#errors.assert.notSpecified('tag'));
		return await this.#instance.delete('/observed/tags/' + tag + '/notifications').then(_ => { return this });
	}
