...
module.exports = class Tag extends API {
	...
	addAuthor = async function({ tag = this.name, username = null } = {}) {
		...
		return this.wrapContent('none', this.#instance.post('/tags/' + tag + '/users/' + username)).then(_ => this);
	}

	removeAuthor = async function({ tag = this.name, username = null } = {}) {
		assert(tag, this.#errors.assert.notSpecified('tag'));
		assert(username, this.#errors.assert.notSpecified('username'));
		return this.wrapContent('none', this.#instance.delete('/tags/' + tag + '/users/' + username)).then(_ => this);
	}
	...
	observe = async function({ tag = this.name } = {}) {
		assert(tag, this.#errors.assert.notSpecified('tag'));
		return this.wrapContent('none', this.#instance.post('/observed/tags/' + tag)).then(_ => this);
	}

	unobserve = async function({ tag = this.name } = {}) {
		assert(tag, this.#errors.assert.notSpecified('tag'));
		return this.wrapContent('none', this.#instance.delete('/observed/tags/' + tag)).then(_ => this);
	}

	notify = async function({ tag = this.name } = {}) {
		assert(tag, this.#errors.assert.notSpecified('tag'));
		return this.wrapContent('none', this.#instance.put('/observed/tags/' + tag + '/notifications')).then(_ => this);
	}

	mute = async function({ tag = this.name } = {}) {
		assert(tag, this.#errors.assert.notSpecified('tag'));
		return this.wrapContent('none', this.#instance.delete('/observed/tags/' + tag + '/notifications')).then(_ => this);
	}
}