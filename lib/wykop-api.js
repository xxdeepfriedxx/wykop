...
module.exports = class API {    
	...
	wrapListing(classString, contentPromise) {
		...
		return Promise.resolve(contentPromise).then(content => {
			...
			console.log('WrapListing Error:', error)
	...
	wrapContent(classString, contentPromise) {
		...
		return Promise.resolve(contentPromise).then(content => {
			...
			console.log('wrapContent Error:', error)
			return contentPromise
		})
	}

	wrapListingMixed(content) {
		if (!contentPromise) { return null }
		return Promise.resolve(contentPromise).then(content => {
			...
			console.log('wrapListingMixed Error:', error)
			return contentPromise
		})
	}
