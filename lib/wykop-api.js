...
module.exports = class API {    
	...
	wrapListing(classString, contentPromise, context, overrideProxy) {
		if (typeof contentPromise.then === 'function') {
			return contentPromise.then(content => this.#_wrapListing(classString, content, context));
		}
		return this.#_wrapListing(classString, contentPromise, context, overrideProxy);
	}

	#_wrapListing(classString, content, context, overrideProxy = false) {
		if (!content) { return []; }
		...
		if (this.#core.useProxies && !overrideProxy) { return proxymise(content); }
		return content;
	}

	wrapContent(classString, contentPromise, context, overrideProxy) {
		if (typeof contentPromise.then === 'function') {
			return contentPromise.then(content => this.#_wrapContent(classString, content, context));
		}
		return this.#_wrapContent(classString, contentPromise, context, overrideProxy);
	}

	#_wrapContent(classString, content, context, overrideProxy = false) {
		...
		if (this.#core.useProxies && !overrideProxy) { return proxymise(parsedContent); }
	...
	wrapListingMixed(contentPromise) {
		...
		return this.#_wrapListingMixed(contentPromise);
	}
