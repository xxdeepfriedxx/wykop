...
| Option                | Description |
| --- | --- |
| `config.appkey`       | <optional> The appkey you received from Wykop |
| `config.secret`       | <optional> The secret you received from Wykop |
| `config.token`        | <optional> Your access token |
| `config.rtoken`       | <optional> Your refresh token |
| `config.environment`  | <optional> The environment (we probably never need this) |
| `config.proxies`      | <optional> Proxies allow for Promise chaining but you can turn them off here |
| `config.debug`        | <optional> Mostly just logs the requests and responses we get from the API |
